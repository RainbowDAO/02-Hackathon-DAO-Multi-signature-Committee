<template>
  <div class="committeeDetail">
    <div class="gov-nav-list">
      <div class="item" style="cursor: pointer;" @click="$router.push({name:'committeeDetail'})">
      ...
      <div class="item" style="cursor: pointer;" @click="$router.push({name:'members'})">
      ...
      <div class="item">
        ...
        <div class="item-content">
          ...
          <div class="number">
            <el-button size="small"  class="button1" @click="$router.push({name:'daoList'})">GO</el-button>
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    toProposal(item){
      this.$router.push({
        name:"proposal",
        params: this.proposalsArr[item.index-1]
      })
    },
    startVote() {
    ...
    getData() {
      ...
      this.$store.dispatch("daoFactory/getUserDaoLength").then(length => {
        ...
        this.$store.dispatch("daoFactory/getUserDaoByIndex", length - 1).then(address => {
          ...
          this.tokenProposals()
          this.$store.commit("daoFactory/SET_DAOADDRESS", this.daoAddress)
